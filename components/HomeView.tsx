...
import type { SearchOptions, SourceApi, WaifuImage } from '../types';
...
export const HomeView: React.FC<HomeViewProps> = ({ currentOptions, onSearchSubmit, onRequestNsfw, onAuthRequest, onNavigateHome, onNavigateComfyUI }) => {
    ...
    // Fetch carousel images whenever isNsfwEnabled toggles
    useEffect(() => {
        let mounted = true;
        const loadCarousel = async () => {
            try {
                // Pre-fetch 15 random images for the carousel display dynamically based on the NSFW setting 
                const random = await getRandomImages(15, { ...localOptions, limit: 15 });
                if(mounted) setCarouselImages(random);
            } catch(e) {
                console.error(e);
            }
        };
    ...
    const toggleNsfwGlobal = () => {
        if (!localOptions.isNsfwEnabled) {
            ...
            // Can toggle it back off natively if wanted
    ...
    return (
        <div className="w-full min-h-screen relative flex flex-col items-center justify-center bg-neutral-50 dark:bg-[#0a0a0a] bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] transition-colors duration-500 overflow-hidden from-violet-100/50 via-neutral-50 to-neutral-50 dark:from-[#1f1035] dark:via-[#0a0a0a] dark:to-[#0a0a0a] p-6">
            ...
            <nav className="absolute top-0 inset-x-0 p-6 flex justify-between items-center z-20">
                ...
                <div className="flex items-center gap-4">
                    ...
                    {user ? (
                        <div className="flex items-center gap-2 group cursor-pointer bg-white dark:bg-[#151515] border border-black/10 dark:border-white/10 px-4 py-1.5 rounded-full shadow-md">
                            <span className="text-sm font-bold text-gray-700 dark:text-gray-300">{user.username}</span>
                        ...
                        <button 
                            ...
                            className="px-5 py-2 bg-black dark:bg-violet-600 hover:bg-neutral-800 dark:hover:bg-violet-700 text-white rounded-full text-sm font-bold transition shadow-lg shadow-black/20 dark:shadow-violet-900/40"
                        >
                            Log/In • Join
                        </button>
                    )}
            ...
            <div className="w-full max-w-2xl text-center flex flex-col items-center animate-fade-in z-10">
                {/* Logo Area — clickable to go back home */}
                <div className="mb-10 flex flex-col items-center">
                    <button
                        ...
                        title="Back to Home"
                    >
                        <svg width="64" height="64" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="mb-6 drop-shadow-[0_0_15px_rgba(139,92,246,0.3)] dark:drop-shadow-[0_0_15px_rgba(139,92,246,0.5)] group-hover:scale-110 transition-transform duration-300">
                        ...
                        <h1 className="text-5xl sm:text-6xl font-black tracking-tighter text-gray-900 dark:text-white mb-2 group-hover:opacity-80 transition-opacity">
                ...
                {/* Search Bar */}
                <form onSubmit={handleSubmit} className="w-full relative group mb-10">
                    <div className="absolute inset-y-0 left-5 flex items-center pointer-events-none">
                        <svg className="w-6 h-6 text-violet-500/50 dark:text-violet-400/50 group-focus-within:text-violet-600 dark:group-focus-within:text-violet-400 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                    </div>
                    <input
                        type="text"
                        value={localQuery}
                        onChange={(e) => setLocalQuery(e.target.value)}
                        className="w-full bg-white dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-full py-4 sm:py-5 pl-14 pr-6 text-lg text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-violet-500/30 focus:border-violet-500 focus:bg-white dark:focus:bg-white/10 transition-all shadow-xl shadow-black/5 dark:shadow-black/50 backdrop-blur-md"
                        placeholder="Search tags or character names..."
                    />
                    <button 
                        type="submit" 
                        className="absolute right-2 top-2 bottom-2 px-6 bg-violet-600 hover:bg-violet-700 text-white font-bold rounded-full transition shadow-lg shadow-violet-500/30 dark:shadow-violet-900/30"
                    >
                        Explore
                    </button>
                </form>
                ...
                <div className="w-full max-w-lg mb-8">
                    <div className="grid grid-cols-4 gap-2 bg-white dark:bg-[#111] p-1.5 rounded-2xl border border-black/5 dark:border-white/5 shadow-md">
                        {(['all', 'images', 'videos', 'gifs'] as const).map(type => (
                            <button
                                key={type}
                                type="button"
                                onClick={() => setContentType(type)}
                                className={`py-2 rounded-xl text-sm font-bold capitalize transition-all ${localOptions.contentType === type ? 'bg-gray-100 dark:bg-[#222] text-violet-600 dark:text-white shadow-sm' : 'text-gray-500 hover:text-gray-800 dark:text-gray-500 dark:hover:text-gray-300'}`}
                            >
                                {type}
                            </button>
                        ))}
                ...
                <div className="flex flex-wrap items-center justify-center gap-3 mb-10">
                    <span className="text-sm font-bold text-gray-500 dark:text-gray-400 mr-2">Sources:</span>
                    {(['waifu.im', 'gelbooru', 'danbooru', 'rule34', 'konachan', 'yandere'] as const).map(source => (
                        ...
                        >
                            <img src={API_FAVICONS[source]} alt={source} className="w-5 h-5 object-contain drop-shadow-sm" />
                        </button>
                    ))}
                </div>

                {/* Carousel — below sources */}
                <div className="w-[120vw] -ml-[10vw] mb-4 overflow-hidden relative">
                    <div className="absolute inset-y-0 left-0 w-32 bg-gradient-to-r from-neutral-50 dark:from-[#0a0a0a] to-transparent z-10 pointer-events-none" />
                    <div className="absolute inset-y-0 right-0 w-32 bg-gradient-to-l from-neutral-50 dark:from-[#0a0a0a] to-transparent z-10 pointer-events-none" />

                    <div className="flex w-max animate-marquee-slow space-x-4 px-4 hover:[animation-play-state:paused]">
                        {carouselImages.length > 0 ? (
                            [...carouselImages, ...carouselImages].map((img, idx) => (
                                <div key={`${img.id}-${idx}`} className="h-40 w-28 shrink-0 rounded-2xl overflow-hidden shadow-lg border border-black/5 dark:border-white/5 bg-gray-200 dark:bg-white/5 transition-transform duration-300 hover:scale-105 cursor-pointer">
                                    <img src={img.thumbnailUrl} alt={img.tags[0] || 'waifu'} className="w-full h-full object-cover" loading="lazy" />
                                </div>
                            ))
                        ) : (
                            // Skeleton placeholders while loading
                            Array.from({ length: 14 }).map((_, idx) => (
                                <div key={idx} className="h-40 w-28 shrink-0 rounded-2xl bg-gray-200 dark:bg-white/5 animate-pulse" />
                            ))
                        )}
                    </div>
                </div>
